...
class BST(object):
    ...
    def findRecursive(self, item, root):
        ...
        if root.item < item:
            return self.findRecursive(item, root.right)
        
        else:
            return self.findRecursive(item, root.left)
    ...
    def preOrder(self, root, lis=[]):
        """
        takes a list and returns it in a preOrder list
        pre: visit the root, then left, then right
        post: returns the list in pre-order
        """

        if root is not None:
            lis.append(root.item)
    ...
    def postOrder(self, root):
        """
        takes a list and returns it in a postorder list
        pre: visit left, then right and then root
        post: returns the list in post-order
        """

        if root is not None:
            for i in self.postOrder(root.left):
                yield i
            for k in self.postOrder(root.right):
                yield k
            yield root.item
...
##        return lis 
    ...
    def __len__(self):
        """
        retuns the len of the tree list
        """
