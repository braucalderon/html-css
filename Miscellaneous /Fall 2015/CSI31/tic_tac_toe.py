...
def main():
     ...
     reset=Button(win,Point(85,13), 23, 8, "Reset")
     reset.activate()
     

     board = [door2,door3,door4,door5,door6,door7,door8,door9,door10, door11]

     pt=win.getMouse()
     while not door11.clicked(pt):


         if (door2.clicked(pt) or door3.clicked(pt) or door4.clicked(pt)
             or door5.clicked(pt) or door6.clicked(pt) or door7.clicked(pt)
             or door8.clicked(pt) or door9.clicked(pt) or door10.clicked(pt)):
                 label=Text(Point(50,85),'Select the Players First').draw(win)
